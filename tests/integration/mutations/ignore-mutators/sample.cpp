...
RUN: unset TERM; %mull_runner -reporters=IDE %s.exe  2>&1 | %filecheck %s --dump-input=fail
RUN: unset TERM; env MULL_CONFIG=%S/mull-group.yml %mull_runner -reporters=IDE %s-group.exe  2>&1 | %filecheck %s --dump-input=fail
