...
RUN: unset TERM; %mull_runner -reporters=IDE %s.exe  2>&1 | %filecheck %s --dump-input=fail --check-prefix=MULL_RUNNER

RUN: %clang_cxx %sysroot -O0 -c -emit-llvm -g -grecord-command-line %s -o - | %mull_instrument | %clang_cxx %sysroot -x ir - -o %s-pipe.exe
RUN: unset TERM; %mull_runner -reporters=IDE %s-pipe.exe  2>&1 | %filecheck %s --dump-input=fail --check-prefix=MULL_RUNNER
