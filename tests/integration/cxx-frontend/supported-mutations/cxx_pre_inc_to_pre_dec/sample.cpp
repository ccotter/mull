...
RUN: (env "cxx_pre_inc_to_pre_dec:%s:7:3:7:5"=1 %s.exe || true) | %filecheck %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITH_MUTATION
...
MULL_RUNNER:{{.*}}sample.cpp:7:3: warning: Killed: Replaced ++ with -- [cxx_pre_inc_to_pre_dec]
*/
