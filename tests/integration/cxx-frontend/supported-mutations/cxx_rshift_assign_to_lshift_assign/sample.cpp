...
int bitwise_rshift_assign(int a, int b) {
  int res = a;
  res >>= b;
  return res;
}
...
int main() {
  if (bitwise_rshift_assign(4, 2) == 1) {
    printf("NORMAL\n");
    return 0;
  } else {
    printf("MUTATED\n");
...
RUN: (env "cxx_rshift_assign_to_lshift_assign:%s:7:7:7:10"=1 %s.exe || true) | %filecheck %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITH_MUTATION
...
MULL_RUNNER:{{.*}}sample.cpp:7:7: warning: Killed: Replaced >>= with <<= [cxx_rshift_assign_to_lshift_assign]
*/
