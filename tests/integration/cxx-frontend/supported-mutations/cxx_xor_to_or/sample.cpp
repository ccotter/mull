...
RUN: (env "cxx_xor_to_or:%s:6:12:6:13"=1 %s.exe || true) | %filecheck %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITH_MUTATION
...
MULL_RUNNER:{{.*}}sample.cpp:6:12: warning: Killed: Replaced ^ with | [cxx_xor_to_or]
*/
