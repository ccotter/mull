...
RUN: (env "cxx_eq_to_ne:%s:6:12:6:14"=1 %s.exe || true) | %filecheck %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITH_MUTATION
...
MULL_RUNNER:{{.*}}sample.cpp:6:12: warning: Killed: Replaced == with != [cxx_eq_to_ne]
*/
