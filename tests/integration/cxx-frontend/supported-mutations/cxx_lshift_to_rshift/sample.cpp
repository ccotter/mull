...
int bitwise_lshift(int a, int b) {
  return a << b;
}

int main() {
  if (bitwise_lshift(2, 4) == 32) {
    printf("NORMAL\n");
    return 0;
  } else {
    printf("MUTATED\n");
...
RUN: (env "cxx_lshift_to_rshift:%s:6:12:6:14"=1 %s.exe || true) | %filecheck %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITH_MUTATION
...
MULL_RUNNER:{{.*}}sample.cpp:6:12: warning: Killed: Replaced << with >> [cxx_lshift_to_rshift]
*/
