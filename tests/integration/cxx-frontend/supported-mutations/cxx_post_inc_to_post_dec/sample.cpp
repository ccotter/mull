...
RUN: (env "cxx_post_inc_to_post_dec:%s:7:6:7:8"=1 %s.exe || true) | %filecheck %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITH_MUTATION
...
MULL_RUNNER:{{.*}}sample.cpp:7:6: warning: Killed: Replaced ++ with -- [cxx_post_inc_to_post_dec]
*/
