...
RUN: (env "cxx_assign_const:%s:7:7:7:14"=1 %s.exe || true) | %filecheck %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITH_MUTATION
...
MULL_RUNNER:{{.*}}sample.cpp:7:7: warning: Killed: Replaced = a + b with 42 [cxx_assign_const]
*/
