...
RUN: (env "cxx_add_assign_to_sub_assign:%s:7:7:7:9"=1 %s.exe || true) | %filecheck %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITH_MUTATION
...
MULL_RUNNER:{{.*}}sample.cpp:7:7: warning: Killed: Replaced += with -= [cxx_add_assign_to_sub_assign]
*/
