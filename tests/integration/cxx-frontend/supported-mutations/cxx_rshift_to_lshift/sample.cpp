...
int bitwise_rshift(int a, int b) {
  return a >> b;
}

int main() {
  if (bitwise_rshift(4, 2) == 1) {
    printf("NORMAL\n");
    return 0;
  } else {
    printf("MUTATED\n");
...
RUN: (env "cxx_rshift_to_lshift:%s:6:12:6:14"=1 %s.exe || true) | %filecheck %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITH_MUTATION
...
MULL_RUNNER:{{.*}}sample.cpp:6:12: warning: Killed: Replaced >> with << [cxx_rshift_to_lshift]
*/
