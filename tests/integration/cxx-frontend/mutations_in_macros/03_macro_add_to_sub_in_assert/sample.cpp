...
#define ASSERT(expr) (!(expr)) ? ((void)printf("MUTATED\n", #expr), exit(1)) : (void)0
...
FRONTEND:Recording mutation point: cxx_add_to_sub:{{.*}}/sample.cpp:9:13:9:14 (end: 9:14)

RUN: %s.exe | %filecheck %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITHOUT_MUTATION
RUN: (env "cxx_add_to_sub:%s:9:13:9:14"=1 %s.exe || true) | %filecheck %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITH_MUTATION
...
MULL_RUNNER:{{.*}}sample.cpp:9:13: warning: Killed: Replaced + with - [cxx_add_to_sub]
*/
